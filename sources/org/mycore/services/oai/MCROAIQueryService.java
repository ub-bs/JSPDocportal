...
public class MCROAIQueryService implements MCROAIQuery {
    ...
    private static final String STR_OAI_SETSCHEME_CLASSID = "MCR.oai.setscheme.classid";
    ...
    public List listSets(String instance) {
        String classificationId = config.getString(STR_OAI_SETSCHEME_CLASSID + "."
                + instance, "");
        String classification[];
        if (classificationId.length() == 0) {
            logger.error("no classification given for " + STR_OAI_SETSCHEME_CLASSID + "." + instance);
            classification = new String[1];
            classification[0] = "DocPortal_class_00000006";
        } else {
    ...
    private List getSets(MCRCategoryItem[] categories, String parentSpec,
        ...
        for (int i = 0; i < categories.length; i++) {
            ...
            int numberOfLinks = ltm.countCategoryReferencesSharp(categories[i]
    ...
    public List getRecord(String id, String metadataPrefix, String instance) {
        ...
        try {
            repositoryId = config.getString(STR_OAI_REPOSITORY_IDENTIFIER + "."
                    + instance);
     ...
     */
    public static String[] getClassifications(String schemetype, String instance)
            throws MCRConfigurationException {
        String classification[];
        String id = config.getString(schemetype + "." + instance);

        if (id.length() == 0) {
            logger.error("kein Klassifikationsfeld spezifiziert in " + schemetype + "." + instance);
            classification = new String[1];
            classification[0] = "id";
        } else {
    ...
    private List listRecordsOrIdentifiers(String[] set, String[] from,
        ...
        String querytype = config.getString(STR_OAI_QUERYTYPE + "." + instance,"document");
...
//                + instance);
        ...
        try {
            String restrictionClassificationField = config
                    .getString(STR_OAI_RESTRICTION_CLASSIFICATION + "."
                            + instance);
            String restrictionCategory = config
                    .getString(STR_OAI_RESTRICTION_CATEGORY + "." + instance);
            
            Element condition = new Element("condition");
            condition.setAttribute("field",restrictionClassificationField);
        ...
        String classification[];
        try {
            classification = getClassifications(STR_OAI_SETSCHEME_FIELD, instance);
        } catch (MCRConfigurationException mcrx) {
        ...
        for (int i = 0; i < classification.length; i++) {
            ...
            if (set == null) {
            	Element condition = new Element("condition");
                condition.setAttribute("field",classification[i]);
