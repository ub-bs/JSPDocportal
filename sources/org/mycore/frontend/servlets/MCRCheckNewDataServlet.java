...
public class MCRCheckNewDataServlet extends MCRCheckDataBase {
	...
	public final String getNextURL(MCRObjectID ID) throws Exception {
		...
		sb.append("nav?path=~workflow-").append(ID.getTypeId());
		return sb.toString();
	}
	...
	public final void sendMail(MCRObjectID ID) throws Exception {
		...
		text.append("Es wurde ein neues Objekt vom Typ ")
				.append(ID.getTypeId()).append(" mit der ID ").append(
						ID.getId()).append(" in den Workflow eingestellt.");
