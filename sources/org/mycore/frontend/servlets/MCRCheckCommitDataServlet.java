...
public class MCRCheckCommitDataServlet extends MCRCheckDataBase {
	...
	public final boolean hasPrivileg(ArrayList privs, String type) {
		if (!(privs.contains("modify-" + type) && privs.contains("commit-"
				+ type)))
			return false;
		return true;
	}
	...
	public final String getNextURL(MCRObjectID ID) throws Exception {
		...
		if (b) {
			...
			// return all is ready
			StringBuffer sb = (new StringBuffer("MCR.type_")).append(
					ID.getTypeId()).append("_in");
			String searchtype = CONFIG.getString(sb.toString(), ID.getTypeId());
			sb = new StringBuffer(
					"servlets/MCRQueryServlet?mode=ObjectMetadata&type=");
			sb.append(searchtype).append(
					"&hosts=local&query=%2Fmycoreobject[%40ID%3D\'").append(
					ID.getId()).append("\']");
			return sb.toString();
		}
		StringBuffer sb = new StringBuffer();
		sb.append(CONFIG.getString("MCR.editor_page_dir", "")).append(
	 ...
	 */
	public final void sendMail(MCRObjectID ID) throws Exception {
		MCRWorkflowManager wfm = MCRWorkflowManager.instance();
		List addr = wfm.getMailAddress(ID.getTypeId(), "wcommit");
		if (addr.size() == 0)
			return;
		String sender = wfm.getMailSender();
		String appl = CONFIG.getString("MCR.editor_mail_application_id",
				"DocPortal");
		String subject = "Automaticaly message from " + appl;
		StringBuffer text = new StringBuffer();
		text.append("Es wurde ein Objekt vom Typ ").append(ID.getTypeId())
				.append(" mit der ID ").append(ID.getId()).append(
