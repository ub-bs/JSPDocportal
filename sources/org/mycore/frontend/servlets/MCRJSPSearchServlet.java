 ...
 */
public class MCRJSPSearchServlet extends MCRServlet {
    private static final long serialVersionUID = 1L;

    private static final Logger LOGGER = Logger.getLogger(MCRJSPSearchServlet.class);
    ...
    private void loadQuery(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    LOGGER.debug("loaded query: \n" + JSPUtils.getPrettyString(query));

        request.setAttribute("query", query);
        request.setAttribute("XSL.Style", "xml");
        request.setAttribute("resultlistType",resultlistType);
        this.getServletContext().getRequestDispatcher("/nav?path=~searchstart-" + resultlistType).forward(request, response);	
    ...
    private void showResults(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    String resultlistType = "simple";
	    Document jdomQuery = new Document(xml);
	    if (query.getRootElement().getAttributeValue("resultlistType") != null) {
	    	resultlistType = query.getRootElement().getAttributeValue("resultlistType");
	    	query.getRootElement().removeAttribute("resultlistType") ;
	    }
        ...
        request.setAttribute("resultlistType",resultlistType);
        this.getServletContext().getRequestDispatcher("/nav?path=~searchresult-" + resultlistType).forward(request, response);	
     ...
     */
    private void doQuery(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String resultlistType = "simple";
	    if (input.getRootElement().getAttributeValue("resultlistType") != null) {
	    	resultlistType = input.getRootElement().getAttributeValue("resultlistType");
	    	input.getRootElement().removeAttribute("resultlistType") ;
	    }
	    
        request.setAttribute("query", input);
        request.setAttribute("resultlistType",resultlistType);
        request.setAttribute("mode","results");
        request.setAttribute("id",result.getID());
        request.setAttribute("numPerPage",npp);
