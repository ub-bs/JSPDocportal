...
public class MCRDefaultPermissionStrategy implements MCRPermissionStrategy {
	...
	private void createReadRuleFromWorkflow(String mcrid, ContextInstance ctxI) {
		...
		String xmlRuleString = ctxI.getVariable(
				MCRWorkflowConstants.WFM_VAR_READRULE_XMLSTRING).toString();
		String ruletype = ctxI.getVariable(
				MCRWorkflowConstants.WFM_VAR_READRULE_TYPE).toString();
		if (ruletype == null) {
			ruletype = "public";
		}
		...
		if (xmlRuleString == null) {
			xmlRuleString = MCRConfiguration
					.instance()
					.getString("MMCR.AccessRuleEditor.defaultrules." + ruletype,
							   "<condition format=\"xml\"><boolean operator=\"true\" /></condition>");
		}

		Element eRule = (Element) MCRXMLHelper.parseXML(xmlRuleString)
				.getRootElement().detach();
		...
		while (it.hasNext()) {
			if (!((String) it.next())
					.startsWith(MCRWorkflowConstants.ACCESSRULE_PREFIX + "_"+ ruletype.toUpperCase())) {
				it.remove();
			}
		}
		...
		if (ruleIDs.size() == 0) {
			...
			String ruleID = MCRWorkflowConstants.ACCESSRULE_PREFIX + "_"+ ruletype.toUpperCase()+ "_"
					+ nrFormat.format(rstore.getNextFreeRuleID(MCRWorkflowConstants.ACCESSRULE_PREFIX
											+ "_"+ ruletype.toUpperCase()+ "_"));
