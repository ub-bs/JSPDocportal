...
public class MCRDefaultPermissionStrategy implements MCRPermissionStrategy{
	...
	public void setPermissions(String mcrid, String userid, String workflowProcessType, int mode){
		...
		if(mode==MCRWorkflowConstants.PERMISSION_MODE_PUBLISH){
			String ruleID ="";
////			if(ctxInst!=null){
////				ruleID=(String)ctxInst.getVariable(MCRWorkflowConstants.WFM_VAR_READRULE);
////			}
////			else{
////				MCRWorkflowManager wfm = MCRWorkflowManagerFactory.getImpl(pid);
////				ruleID=wfm.getStringVariable(MCRWorkflowConstants.WFM_VAR_READRULE, pid);
////			}
//
//			if(ruleID!=""){
//				MCRRuleStore rs = MCRRuleStore.getInstance();
...
//				bool.addContent(rs.getRule(ruleID).getRule().toXML());
...
//
//				MCRRuleMapping ruleMapping = ACS.getAutoGeneratedRuleMapping(rule, "RuleEditor", "read", mcrid, "");
//				accessStore.updateAccessDefinition(ruleMapping);
//				
//				
//				//MCRAccessControlSystem ACS = (MCRAccessControlSystem)MCRAccessControlSystem.instance();
//				//strRule = ACS.getNormalizedRuleString(eRule);
//			//	if(!strRule.startsWith("(")){strRule="("+strRule+")";}
//			}
//			else{
...
//			}
			...
			MCRAccessStore accessstore = MCRAccessStore.getInstance();
			ruleID= accessstore.getRuleID(mcrid, MCRWorkflowAccessRuleEditorUtils.EDITPOOL);
			if(ruleID!=""){
				MCRRuleMapping rm = accessstore.getAccessDefinition(ruleID, MCRWorkflowAccessRuleEditorUtils.EDITPOOL, mcrid);
				accessstore.deleteAccessDefinition(rm);
				rm.setPool("read");
				accessstore.updateAccessDefinition(rm);
			}
			else{
				String strRule =config.getString("MCR.AccessRule.STANDARD-READ-RULE","<condition format=\"xml\"><boolean operator=\"true\" /></condition>");
				 Element rule = (Element)MCRXMLHelper.parseXML(strRule).getRootElement().detach();
...
//			String oRuleID = MCRAccessStore.getInstance().getRuleID(mcrid, "read");
//			MCRWorkflowManager wfm = MCRWorkflowManagerFactory.getImpl(pid);
//			if(oRuleID.startsWith(MCRWorkflowAccessRuleEditorUtils.RULEPREFIX)){
//				if(ctxInst!=null){
//					ctxInst.setVariable(MCRWorkflowConstants.WFM_VAR_READRULE, oRuleID);
//				}
//				else{
//					wfm.setStringVariable(MCRWorkflowConstants.WFM_VAR_READRULE, oRuleID, pid);
//				}
//			}
//			else{
//				if(ctxInst!=null){
//					ctxInst.setVariable(MCRWorkflowConstants.WFM_VAR_READRULE, "");
//				}
//				else{
//					wfm.setStringVariable(MCRWorkflowConstants.WFM_VAR_READRULE, "", pid);
//				}
//			}
//			
//			//+++++++++++++++++++++++++++++++++++++++++++
...
//			MCRAccessStore accessstore = MCRAccessStore.getInstance();
//			String oRuleID= accessstore.getRuleID(mcrid, MCRWorkflowAccessRuleEditorUtils.EDITPOOL);
//			if(oRuleID!=""){
//				MCRRuleMapping rm = accessstore.getAccessDefinition(oRuleID, MCRWorkflowAccessRuleEditorUtils.EDITPOOL, mcrid);
//				rm.setPool(MCRWorkflowAccessRuleEditorUtils.EDITPOOL);
//				accessstore.updateAccessDefinition(rm);
//			}
//			else{
//				String strRule =config.getString("MCR.AccessRule.STANDARD-READ-RULE","<condition format=\"xml\"><boolean operator=\"true\" /></condition>");
//				 Element rule = (Element)MCRXMLHelper.parseXML(strRule).getRootElement().detach();
//				if(AI.hasRule(mcrid,  MCRWorkflowAccessRuleEditorUtils.EDITPOOL)){
//					AI.updateRule(mcrid,  MCRWorkflowAccessRuleEditorUtils.EDITPOOL, rule, "");
//				}else{
//					AI.addRule(mcrid,  MCRWorkflowAccessRuleEditorUtils.EDITPOOL, rule, "");
//				}
//			}		
		}
