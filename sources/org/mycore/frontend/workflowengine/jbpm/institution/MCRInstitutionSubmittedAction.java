...
public class MCRInstitutionSubmittedAction extends MCRAbstractAction {
	...
	private static Logger logger = Logger.getLogger(MCRInstitutionSubmittedAction.class);
	private static MCRAccessInterface AI = MCRAccessManager.getAccessImpl();
	private static Element editorModeRule;
	private static Element institutionReadRule;
	private static String strReadRule;
	
	static{  
		String strRule = MCRConfiguration.instance().getString("MCR.WorkflowEngine.defaultACL.editorMode.institution", "<condition format=\"xml\"><boolean operator=\"or\"><condition field=\"group\" operator=\"=\" value=\"adminInstitution\" /></boolean></condition>");
		editorModeRule = (Element)MCRXMLHelper.parseXML(strRule).getRootElement().detach();
		strReadRule = MCRConfiguration.instance().getString("MCR.WorkflowEngine.defaultACL.institution.read.institution");	
	}
	...
	public void executeAction(ExecutionContext executionContext) throws MCRException {
		...
		institutionReadRule = (Element)MCRXMLHelper.parseXML(x).getRootElement().detach();
		...
		for (Iterator it = ids.iterator(); it.hasNext();) {
			...
			for (Iterator it2 = permissions.iterator(); it2.hasNext();) {
				...
				AI.addRule(id, "read", institutionReadRule, "");
		}
	}
