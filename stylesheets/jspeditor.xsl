...
<xsl:template name="editor.set.form.attrib">    
  ...
  <xsl:if test="descendant::file">
    ...
    <xsl:attribute name="action">
      <xsl:value-of select="concat('upload',$HttpSession)"/>
    </xsl:attribute>
  </xsl:if>
  <xsl:if test="not(descendant::file)">
    <xsl:attribute name="action">
      <xsl:value-of select="concat('nav',$HttpSession,'?path=~editor')"/>
    </xsl:attribute>
  </xsl:if>
  ...
  <td align="left" valign="bottom">
    <xsl:if test="$num &lt; $max" >
      <div style="margin-left:2px; margin-bottom:2px;">
        <input type="image" name="{$editor.delimiter.internal}p-{$var}-{$row.nr}" src="{$WebApplicationBaseURL}img/pmud-plus.png"/>
      </div>
    </xsl:if>
  </td>
  <td align="left" valign="bottom">
      <div style="margin-left:2px;  margin-bottom:2px;">
        <input type="image" name="{$editor.delimiter.internal}m-{$var}-{$row.nr}" src="{$WebApplicationBaseURL}img/pmud-minus.png"/>
      </div>
  </td>
  <td align="left" valign="bottom">
    <xsl:if test="($row.nr &lt; $num) or ($row.nr &lt; $min)" >
      <div style="margin-left:2px;  margin-bottom:2px;">
        <input type="image" name="{$editor.delimiter.internal}d-{$var}-{$row.nr}" src="{$WebApplicationBaseURL}img/pmud-down.png"/>
      </div>
    </xsl:if>
  </td>
  <td align="left" valign="bottom">
    <xsl:if test="$row.nr &gt; 1" >
      <div style="margin-left:2px;  margin-bottom:2px;">
        <input type="image" name="{$editor.delimiter.internal}u-{$var}-{$row.nr}" src="{$WebApplicationBaseURL}img/pmud-up.png"/>
      </div>
    </xsl:if>
  </td>
</xsl:template>
...
<xsl:template match="cancelButton">
  ...
  <xsl:variable name="url_new" select="concat('nav',$HttpSession,'?path=~editor','&amp;workflow=cancel')" />

  <!-- ======== output cancel button ======== -->
  <xsl:call-template name="editor.button">
    <xsl:with-param name="url"   select="$url_new"   />
