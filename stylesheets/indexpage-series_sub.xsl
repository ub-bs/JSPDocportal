...
<!-- ============================================== -->
<!-- $Revision: 1.16 $ $Date: 2008-01-11 16:57:42 $ -->
<!-- ============================================== -->
  ...
  <tr>
    <td>
      <table border="0" cellpadding="0" cellspacing="0"><tr>
        <td class="metavalue">
          <form action="{$WebApplicationBaseURL}nav?path=~indexSearchSeriesSub" method="post">
            <b>Index </b>
...
<xsl:variable name="up.url">
	<xsl:text>nav?path=~indexSearchSeriesSub</xsl:text>
...
<xsl:template match="range">
  <xsl:variable name="url">
    <xsl:value-of select="concat($WebApplicationBaseURL,'nav?path=~indexSearchSeriesSub&amp;fromTo=', from/@pos,'-', to/@pos, '&amp;prevFromTo=', $upFromTo, '.', $prevFromTo)" />
    <xsl:text>&amp;</xsl:text>
...
Dieser Index enthält die Mehrbändigen Werke und Zeitschriften in der Digitalen Bibliothek.
<br/><br/>
...
<xsl:for-each select="xalan:nodeset($AtoZ)/search">
 ...
 <a href="{$WebApplicationBaseURL}nav?path=~indexSearchSeriesSub&amp;search={@prefix}&amp;{$subselect.params}">
