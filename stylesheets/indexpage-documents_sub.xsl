...
<xsl:variable name="up.url">
	<xsl:text>nav?path=~indexSearchDocumentsSub</xsl:text>
...
<xsl:template match="range">
  <xsl:variable name="url">
    <xsl:value-of select="concat($WebApplicationBaseURL,'nav?path=~indexSearchDocumentsSub&amp;fromTo=', from/@pos,'-', to/@pos, '&amp;prevFromTo=', $upFromTo, '.', $prevFromTo)" />
    <xsl:text>&amp;</xsl:text>
...
<xsl:for-each select="xalan:nodeset($AtoZ)/search">
 ...
 <a href="{$WebApplicationBaseURL}nav?path=~indexSearchDocumentsSub&amp;search={@prefix}&amp;{$subselect.params}">
