...
public class MCRWorkflowManagerThesis extends MCRWorkflowManager{
	
	
	private static Logger logger = Logger.getLogger(MCRWorkflowManagerThesis.class.getName());
	...
	protected MCRWorkflowManagerThesis() throws Exception {
		super("thesis", "thesis");
	...
	public static synchronized MCRWorkflowManager instance() throws Exception {
		if (singleton == null)
			singleton = Guice.createInjector(new MCRThesisWorkflowModule()).getInstance(MCRWorkflowManager.class);
		return singleton;
	}
...
//		allow multiple workflowproccesses - same code as in MCRWorkflowManagerPublication		
	...
	public String checkDecisionNode(String decisionNode, ContextInstance ctxI) {
		if(decisionNode.equals("canThesisBeSubmitted")){
			if(checkSubmitVariables(ctxI)){
				return "thesisCanBeSubmitted";
			}else{
				return "thesisCantBeSubmitted";
			}
		}else if(decisionNode.equals("canThesisBeCommitted")){
			if(checkSubmitVariables(ctxI)){
				...
				return "go2sendBackToThesisCreated";
			}
		}
