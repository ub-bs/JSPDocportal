...
import org.mycore.frontend.workflowengine.guice.MCRSeriesWorkflowModule;
...
public class MCRWorkflowManagerSeries extends MCRWorkflowManager{
	
	
	private static Logger logger = Logger.getLogger(MCRWorkflowManagerSeries.class.getName());
	...
	protected MCRWorkflowManagerSeries() throws Exception {
		super("series", "series, series-volume", "series");
	}
	...
	public static synchronized MCRWorkflowManager instance() throws Exception {
		if (singleton == null)
			singleton = Guice.createInjector(new MCRSeriesWorkflowModule()).getInstance(MCRWorkflowManager.class);
		return singleton;
	}
...
//		allow multiple workflowproccesses - same code as in MCRWorkflowManagerPublication		
	...
	public String checkDecisionNode(String decisionNode, ContextInstance ctxI) {
		if(decisionNode.equals("canSeriesBeSubmitted")){
			if(checkSubmitVariables(ctxI)){
				return "seriesCanBeSubmitted";
			}else{
				return "seriesCantBeSubmitted";
			}
		}
		else if(decisionNode.equals("canVolumeBeSubmitted")){
			if(checkSubmitVariables(ctxI)){
				return "volumeCanBeSubmitted";
			}else{
				return "volumeCantBeSubmitted";
			}
		}else if(decisionNode.equals("canSeriesBeCommitted")){
			if(checkSubmitVariables(ctxI)){
				return "go2wasCommitmentSuccessful";
				
			}else{
				return "go2sendBackToSeriesCreated";
			}
		}
		
		else if(decisionNode.equals("canVolumeBeCommitted")){
			if(checkSubmitVariables(ctxI)){
				return "go2wasVolumeCommitmentSuccessful";
			}else{
				return "go2sendBackToVolumeCreated";
			}
		}
		return null;
	}
