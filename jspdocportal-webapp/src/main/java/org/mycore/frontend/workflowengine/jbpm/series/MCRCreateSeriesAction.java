...
public class MCRCreateSeriesAction extends MCRAbstractAction{
	...
	public void executeAction(ExecutionContext executionContext) throws MCRException {
		logger.debug("creating empty series");
		executionContext.setVariable(MCRWorkflowConstants.WFM_VAR_METADATA_PUBLICATIONTYPE, "series");	
		...
		if(createdDocID != null && !createdDocID.equals("")){
			...
			String errMsg = "could not create a docID for series";
			logger.error(errMsg);
