...
<xsl:variable name="applet.codebase"          select="concat($WebApplicationBaseURL,'applets')" />
...
<xsl:template match="fileupload">
  <xsl:variable name="url">  <!-- when applet ends this is shown --> 
    <xsl:value-of select="concat($WebApplicationBaseURL,'servlets/MCRUploadServlet?method=redirecturl&amp;uploadId=',$UploadID)"/>
  </xsl:variable>
<center>  
<table style="border-style:groove; border-width: 2px; border-color:#000000;" >
   <tr>
      <td>
  <object 
